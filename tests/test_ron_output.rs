...
fn test_ron_output_for_non_compliant() {
    ...
    assert!(
        ...
        "Output did not contain diagnostics:: {}",
        stdout
    );
    assert!(
        stdout.contains("file:"),
        "Output did not contain file:: {}",
        stdout
    );
    assert!(
        stdout.contains("severity: Error"),
        "Output did not contain severity: Error: {}",
        stdout
    );
    assert!(
        stdout.contains("code:"),
        "Output did not contain code:: {}",
        stdout
    );
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "Expected exit code 1, but got something else. Output: {}",
        stdout
    );
}
