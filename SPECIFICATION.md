...
- 在文件头部，声明顺序必须为：
  ...
  3. `mod` 模块声明。
  4. `use` 导入语句。
  5. 其他代码项（Struct, Enum, Function 等）。
...
### 3.1 禁止简单逻辑的函数别名
禁止创建参数类型、返回类型以及内部逻辑完全等价的简单函数/方法包装（Alias）。
- **违规条件**：当一个函数仅包含对另一个函数或方法的单一调用，且调用参数与函数签名参数完全一致（顺序与名称一致，包含 `self`）时，若被调用方属于本 crate 且不是 Trait 方法，则视为违规。
- **允许特例**：
  - 被调用的函数/方法不在当前 crate 中（如标准库、外部 dependency）。
  - 被调用的函数/方法是一个 Trait 方法。
- **判定机制**：工具通过全局索引自动识别工作空间内的固有逻辑与 Trait 接口，不再依赖硬编码白名单或简单的路径路径解析。
- **反例**：
  ```rust
  ...
  // 错误：简单包装本 crate 内部方法
  pub fn update_pool(p: &mut Pool, idx: usize) { p.update(idx) }
  ```
- **正例**：
  ```rust
  // 正确：包装外部 Crate 函数
  fn wrap_external(a: i32) { other_crate::func(a) }

  // 正确：包装 Trait 方法
  fn wrap_trait(s: &str) -> String { s.to_string() }
  ```
